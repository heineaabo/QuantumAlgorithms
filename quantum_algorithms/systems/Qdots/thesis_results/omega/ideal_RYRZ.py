...
option_noisy = {'shots':shots,'print':False,
            ...
            'coupling_map':True,'layout':[0,2,1,3],'meas_fit':True}
...
for omega in tqdm(omegas):
    ...
    # Noisy
    model = VQE(h2,Minimizer('Cobyla',tol=1/(100*shots),disp=False),'RYRZ',ansatz_depth=2,options=option_noisy)
    theta = model.optimize()
    Es_noisy[i],VARs_noisy[i] = model.get_mean(theta,N=10000,M=10)
    vqe_coeffs_noisy[i] = model.get_state_coeff(theta)
    i += 1

np.save('RYRZ/bonds.npy',omegas)
np.save('RYRZ/fci.npy',FCIs)
np.save('RYRZ/hf.npy',HFs)
np.save('RYRZ/cobyla/E_ideal.npy',Es_ideal)
np.save('RYRZ/cobyla/var_ideal.npy',VARs_ideal)
np.save('RYRZ/cobyla/coeff_ideal.npy',vqe_coeffs_ideal)
np.save('RYRZ/cobyla/E_noisy.npy',Es_noisy)
np.save('RYRZ/cobyla/var_noisy.npy',VARs_noisy)
np.save('RYRZ/cobyla/coeff_noisy.npy',vqe_coeffs_noisy)
