...
class VQE_UCC:
	def __init__(self,n_qubits,n_fermi,circuit_list,noise_model=None,shots=1000):
	...
	def wavefunction_ansatz(self,qc,qb,cb,t,measure=False):
		...
		if measure:
			...
			#job = qk.execute(qc, backend = qk.Aer.get_backend('qasm_simulator'), shots=self.shots,noise_model=self.noise_model)
			result = job.result()
	...
	def non_gradient_optimization(self):
            ...
            res = minimize(self.energy, t, method='COBYLA', options={'disp': True,'maxiter':100},tol=1e-12)
            print(res.x)
...
test = VQE_UCC(n_qubits=n_qubits,n_fermi=n_fermi,circuit_list=circuit_list,noise_model=noise_model)
test.non_gradient_optimization()
np.save('VQE_without.npy',test.plotx)
