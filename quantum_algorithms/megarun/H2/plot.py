...
uccdr_i = np.load('single_param/E_ideal_UCCDr.npy') 
#uccdr_i = uccd_i[[i for i in range(0,len(uccd_i),5)]]
uccdr_n = np.load('single_param/E_noisy_UCCDr.npy') 
...
plt.plot(x2,uccdr_i,'*',label='UCCDr')
...
plt.plot(x2,uccdr_n,'*',label='UCCDr')
...
plt.plot(x2,np.abs(fci2-uccdr_i),'*',label='UCCDr')
