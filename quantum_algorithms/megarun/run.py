...
for R in tqdm(Rs):
    ...
    #### UCC
    # Ideal
    model = VQE(h2,Minimizer('Cobyla',tol=1/(100*shots),disp=False),'UCCD',options=option_ideal)
    theta = model.optimize()
    Es_ideal_UCC[i],VARs_ideal_UCC[i] = model.get_mean(theta,N=10000,M=10)
    vqe_coeffs_ideal_UCC[i] = model.get_state_coeff(theta)
    # Noisy
    model = VQE(h2,Minimizer('Cobyla',tol=1/(100*shots),disp=False),'UCCD',options=option_noisy_RY)
    theta = model.optimize()
...
# UCC
np.save('H2/cobyla/E_ideal_UCC.npy',Es_ideal_UCC)
np.save('H2/cobyla/var_ideal_UCC.npy',VARs_ideal_UCC)
np.save('H2/cobyla/coeff_ideal_UCC.npy',vqe_coeffs_ideal_UCC)
np.save('H2/cobyla/E_noisy_UCC.npy',Es_noisy_UCC)
np.save('H2/cobyla/var_noisy_UCC.npy',VARs_noisy_UCC)
np.save('H2/cobyla/coeff_noisy_UCC.npy',vqe_coeffs_noisy_UCC)
...
for omega in tqdm(omegas):
    ...
    vqe_coeffs_ideal_UCC[i] = model.get_state_coeff(theta)
    # Noisy
    model = VQE(h2,Minimizer('Cobyla',tol=1/(100*shots),disp=False),'UCCD',options=option_noisy_UCC)
    theta = model.optimize()
    Es_noisy_UCC[i],VARs_noisy_UCC[i] = model.get_mean(theta,N=10000,M=10)
    vqe_coeffs_noisy_UCC[i] = model.get_state_coeff(theta)
